...
void loop()
{
  ...
  temp = (float)(myIMU.fifoRead()) * 4.375 * (gyroRangeDivisor) / 1000;
  Serial.print(temp);
  Serial.print(",");
  temp = (float)(myIMU.fifoRead()) * 4.375 * (gyroRangeDivisor) / 1000;
  Serial.print(temp);
  ...
  Serial.print(temp);
  Serial.print(",");
  tempRaw = myIMU.fifoRead();
  float tempAccel = (float)(tempRaw) * 0.061 * (myIMU.settings.accelRange >> 1) / 1000;
  if( tempAccel < 0.01 ) {
    tempAccel = 0.01;
  }
  ...
  Serial.print(",");
  tempRaw = myIMU.fifoRead();
  tempAccel = (float)(tempRaw) * 0.061 * (myIMU.settings.accelRange >> 1) / 1000;
  if( tempAccel < 0.01 ) {
    tempAccel = 0.01;
  }
  Serial.print(tempAccel);
