...
//The n_DIR #defines are actually math offsets for framing returned
//data.  The data is 16 bits so Z_DIR offsets by 4 8-bit bytes
#define X_DIR 0
#define Y_DIR 1
#define Z_DIR 2
...
class Sparkfun_LSM6DS3 {
  ...
  public:
    ...
    void readAccel( void );
    float readAccel( uint8_t );
    void readAccelRaw( void );
    int16_t readAccelRaw( uint8_t );
    void readGyro( void );
    float readGyro( uint8_t );
    void readGyroRaw( void );
    int16_t readGyroRaw( uint8_t );
    int16_t readTemp( void );
    void readAll( void );
    void readAllRaw( void );
